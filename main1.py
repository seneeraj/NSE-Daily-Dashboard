...
# Allow manual input of access token for testing
st.sidebar.header("Update Access Token")
new_access_token = st.sidebar.text_input("Enter new access token (optional):", "")
if new_access_token:
    st.secrets["zerodha"]["access_token"] = new_access_token
    st.write("✅ Updated access token in session. Redeploy with updated secrets.toml for persistence.")
...
4. Update st.secrets or use the sidebar to input the new access_token""")
